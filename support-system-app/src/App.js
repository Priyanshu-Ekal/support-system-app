...
function App() {
  ...
  const logout = () => {
    ...
    axios.put("http://localhost:8080/logout-uuid", {
      email: userEmail,
  ...
  useEffect(() => {
    ...
    if (sessionStorage.getItem("token") && checkedToken === false) {
      axios.get("http://localhost:8080/check-token/" + sessionStorage.getItem("email"), config).then(
        (res) => {
          try {
  ...
  return (
    <React.Fragment>
      <br></br>
      {alert !== "default" ?
        <div className={"alert alert-" + alertType + " animated fadeInUp"} role="alert">{alert}</div>
        : null}
      {isValidUser === false ?
        ...
        <React.Fragment>
          ...
          <div className={activeModule === "timeline" ? "fluid bg-gray-800" : "fluid bg-gray-400"}>
          ...
          <footer className="footer mt-auto py-3 px-3 bg-dark text-muted">
            <div className="row">
              ...
              <div className="col-md-2">
                <button className="bg-gray-600 text-white px-3 py-1.5 border border-primary rounded btn btn-block btn-danger" onClick={() => logout()}>Logout</button>
              </div>
            </div>
          </footer>
        </React.Fragment>
